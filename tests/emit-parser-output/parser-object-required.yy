    ...
    LEFT_BRACE object_items_0 RIGHT_BRACE
        {
            if ($2.find("foo") == $2.end())
            {
                std::cerr << "Object missing required object property: \"foo\"" << std::endl;
                YYABORT;
            }

            if ($2.find("bar") == $2.end())
            {
                std::cerr << "Object missing required object property: \"bar\"" << std::endl;
                YYABORT;
            }
...
object_item_0:
    TOKEN_0 QUOTED_STRING COLON QUOTED_STRING
        {
            $$.set_object_item($2, $4);
        }
    | TOKEN_0 QUOTED_STRING COLON number
        {
            $$.set_object_item($2, $4);
        }
    | TOKEN_0 QUOTED_STRING COLON boolean
        {
            $$.set_object_item($2, $4);
        }
    | TOKEN_0 QUOTED_STRING COLON array
        {
            $$.set_object_item($2, $4);
        }
    | TOKEN_0 QUOTED_STRING COLON object
        {
            $$.set_object_item($2, $4);
        }
    | TOKEN_0 QUOTED_STRING COLON NULL_TOKEN
        {
            $$.set_object_item($2);
        }
    ;
